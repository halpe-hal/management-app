...
from modules.brands_setting import handle_brand_setting

def show_setting_menu():
    st.markdown("## 設定")

    tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs(["事業部・店舗設定", "ブランド設定", "勘定科目設定", "費目設定", "目標比率設定", "取引先デフォルト設定", "入金元デフォルト設定"])
    with tab1:
        handle_division_setting()
    with tab2:
        handle_brand_setting()
    with tab3:
